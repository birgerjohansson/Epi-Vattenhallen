...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    var guessResults = [
      { selectedImage: "", faceRecEmotions: {}, epiEmotion: {}, guesses: [] },
    ]; //guesses = tempExp?

    this.setState({
      emotionsObject: emotionsObject.emotionsObject,
  ...
  setNewEmotion = () => {
    ...
    this.setState({
      expObject: newExpObject.expObject,
  ...
  playEmotion = () => {
    ...
    console.log(
      "http://127.0.0.1:8000/control/MotionTrigger.data/" +
        currEmotion +
        "/0/1 "
    );
    fetch(
  ...
  stopEmotion = () => {
    ...
    for (i = 0; i < 7; i++) {
      ...
      console.log(i);
      fetch(
        "http://127.0.0.1:8000/control/MotionTrigger.data/" + i + "/0/0 "
      ).then((response) => response.json());
    }
  };
  ...
  changeCurrentState = (stateValue) => {
    ...
    this.setState({
      ...
      emotionsObject: tempEmotionsObj,
  ...
  updateExp = (expNumberObject, selectedEmotion) => {
    ...
    selectedEmotion.boolean
      ? guessResults[guessResults.length - 1].guesses.push(selectedEmotion)
      : guessResults[guessResults.length - 1].guesses.splice(
          guessResults[guessResults.length - 1].guesses.indexOf(
            selectedEmotion
          ),
          1
        );
  ...
  setFaceRecEmotions = (emotions) => {
    ...
    guessResults[guessResults.length - 1].selectedImage =
      this.state.selectedImage;
    guessResults[guessResults.length - 1].epiEmotion =
  ...
  render() {
    ...
    const expEyeColorElem = (params) => (
      <ExpEyeColor
        {...params}
        expObject={experiment}
        emotionsObject={emotions}
        currentState={stateValue}
        emotionDisplay={emDisplay}
        callbackFromParent={this.updateExp}
        callbackFromParentExit={this.exitExp}
      />
    );
    ...
    const resultElem = (params) => (
      <Result
        {...params}
        expObject={experiment}
        emotionsList={emotionsList}
        faceRecEmotions={faceRecEmotions}
        selectedImage={selectedImage}
        emotionsObject={emotions}
        guessResults={guessResults}
        callbackSetNewEmotion={this.setNewEmotion}
        callbackFromParent={this.exitExp}
      />
    );
    const summaryElem = (params) => (
      <Summary
        {...params}
        guessResults={guessResults}
        callbackFromParent={this.exitExp}
    ...
    const expMainElem = (params) => (
      <ExpMain {...params} callbackFromParent={this.startExp} />
    );
    const expSelectApproachElem = (params) => (
      <ExpSelectApproach {...params} callbackFromParent={this.exitExp} />
    );
    const expChoosePictureElem = (params) => (
      <ExpChoosePicture {...params} callbackFromParent={this.faceRec} />
    );
    const expTakePictureElem = (params) => (
      <ExpTakePicture {...params} callbackFromParent={this.faceRec} />
    );
    const expPreGameInstructionElem = (params) => (
      <ExpPreGameInstruction
        {...params}
        selectedImage={selectedImage}
        guessResults={guessResults}
        callbackFromParent={this.setFaceRecEmotions}
        callbackFromParentFeedback={this.setUserFeedback}
      />
    );
        ...
        <div>
          ...
          <div className="form-group mt-20">
            <Switch>
              ...
              <Route
                path="/ExpChoosePicture"
                component={expChoosePictureElem}
              />
              <Route
                path="/ExpSelectApproach"
                component={expSelectApproachElem}
              />
              <Route
                path="/ExpPreGameInstruction"
                component={expPreGameInstructionElem}
              />
